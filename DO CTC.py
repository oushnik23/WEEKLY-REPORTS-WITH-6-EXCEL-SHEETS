...
df=df[(df['Category'].isin(['CTC'])) & (df['SalesAlies']==Sale) & (df['AreaAlies'].isin(['DO/TR'])) & 
...
df3=df3.sort_index()
df3 = df3.reindex(pd.Index(['KOL', 'SIL', 'Comb'], name="Centre"), level="Centre")
...
df4=df4.sort_index()
df4 = df4.reindex(pd.Index(['KOL', 'SIL', 'Comb'], name="Centre"), level="Centre")

#--------------------------------------Desired Orders---------------------------------------------------#

# Define desired orders
centre_order = {'KOL': 0, 'SIL': 1, 'Comb': 2}
ps_order = {'P': 0, 'S': 1, '*': 2}
grade_sequence = [
...
gardenlist=df_pc['GardenMDM'][(df_pc['BrokerCode']=='PC') & (df_pc['AreaAlies']=='DO/TR') 
...
#------------------------------------------INSERT BLANK SPACE------------------------------------------#
targets = [('KOL', '*', 'Grand Total'), ('SIL', '*', 'Grand Total')]
...
excel_path = "DO CTC.xlsx"

with pd.ExcelWriter(excel_path, engine='xlsxwriter') as writer:
    df4.to_excel(writer, sheet_name="DO CTC", startrow=3)
    workbook  = writer.book
    worksheet = writer.sheets["DO CTC"]
...
targets = [('KOL', '*', 'Grand Total'), ('SIL', '*', 'Grand Total'), ('Comb', '*', 'Grand Total')]
...
targets1 = [('KOL','P','Total'),('KOL','S','Total'),('SIL','P','Total'),('SIL','S','Total'),
